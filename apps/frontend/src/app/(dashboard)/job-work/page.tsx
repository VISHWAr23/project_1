...
export default function JobWorkProductionHubPage() {
  ...
  const { data: jobWorkOrdersData } = useJobWorkOrders({ limit: 50 });
  ...
  const activeWeavingOrders = useMemo(() => {
    return (
      jobWorkOrdersData?.items?.filter(
        (o: any) =>
          o.jobWorkType === 'WEAVING' &&
          o.status !== 'COMPLETED' &&
          o.status !== 'CANCELLED' &&
          o.status !== 'CLOSED'
      ) || []
  ...
  const allUnifiedBatches = useMemo(() => {
    ...
    activeWeavingOrders.forEach((order: any) => {
      list.push({
        ...
        categoryKey: 'weaving',
        typeLabel: 'Weaving',
        badgeClasses: 'bg-lime-500/10 text-lime-700 dark:text-lime-300 border-lime-500/20',
        textAccentClass: 'text-lime-600 dark:text-lime-400',
        borderHoverClass: 'hover:border-lime-500/50',
        productName: order.weavingDetail
          ? `Woven Fabric (${order.weavingDetail.ends} Ends × ${order.weavingDetail.reed} Reed, ${order.weavingDetail.pick} Pick)`
          : order.jobWorkCompany?.companyName || 'Weaving Job Work',
        dateStr: resolveDisplayDate(order),
        timestamp: resolveTimestamp(order),
        metricPrimary: (
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>Pieces: <strong className="text-foreground font-mono">{order.weavingDetail?.totalPieces || 0} pcs</strong> ({order.weavingDetail?.totalPaavu || 0} Paavu)</span>
            <span className="px-2 py-0.5 rounded bg-secondary text-[10px] font-mono">{order.status}</span>
          </div>
        ),
        metricSecondary: (
          <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground bg-secondary/40 px-2 py-1 rounded">
            <span>Mill: {order.jobWorkCompany?.companyName || 'Subcontractor'}</span>
            <span className="font-semibold text-emerald-600 dark:text-emerald-400 font-mono">
              ₹{Number(order.weavingDetail?.totalSalary || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })} Salary
            </span>
          </div>
        ),
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {/* Card 7: Weaving Job Work */}
          <motion.div
            whileHover={{ y: -2 }}
            transition={{ type: 'spring', stiffness: 400, damping: 25 }}
            onClick={() => handleOpenAssignment('WEAVING')}
            className="group cursor-pointer"
          >
            <Card className="relative overflow-hidden p-4 sm:p-5 h-full flex flex-col justify-between border border-border hover:border-lime-500/60 hover:shadow-md transition-all bg-gradient-to-br from-card via-card to-lime-500/5">
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-lime-500 to-emerald-600" />

              <div>
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-lime-500/10 text-lime-600 dark:text-lime-400 flex items-center justify-center group-hover:bg-lime-600 group-hover:text-white transition-all shadow-2xs shrink-0">
                      <Layers className="h-4.5 w-4.5" />
                    </div>
                    <div>
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-lime-500/10 text-lime-700 dark:text-lime-300 border border-lime-500/20 shrink-0">
                    Formulas & In-Pass
                  </span>
                </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-1.5 text-[11px]">
                  <span className="px-2 py-0.5 rounded bg-secondary text-muted-foreground font-mono">
                    {activeWeavingOrders.length} WIP Orders
                  </span>
                  <span className="px-2 py-0.5 rounded bg-secondary text-muted-foreground font-mono">
                    Yarn Formulas
                  </span>
              ...
              <div className="mt-4 pt-3 border-t border-border/70 flex items-center justify-between">
                ...
                >
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-8 px-2.5 text-xs gap-1.5 border-border hover:border-lime-500/50 hover:bg-lime-500/5 text-muted-foreground hover:text-lime-600 dark:hover:text-lime-400 transition-colors"
                  >
                    <span>Go to Production</span>
                    <ExternalLink className="h-3 w-3" />
                  </Button>
